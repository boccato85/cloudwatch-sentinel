...
# M8: Session with retry adapter for transient network errors.
...
def get_sentinel_summary() -> dict:
    ...
    try:
        resp = _session.get(f"{SENTINEL_URL}/api/summary", timeout=10)
...
def get_sentinel_metrics() -> dict:
    ...
    try:
        resp = _session.get(f"{SENTINEL_URL}/api/metrics", timeout=10)
        resp.raise_for_status()
        return {"data": resp.json(), "error": None}
    except requests.RequestException as exc:
        return {"data": None, "error": str(exc)}
