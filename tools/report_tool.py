...
def validate_and_write(content: str, filepath: str) -> dict:
    ...
    # C6: Verify harness exists before invoking to catch misconfigured deployments early.
    ...
    if result.returncode != 0:
        # I5: Sanitize and limit error message to prevent reflecting untrusted harness output.
        raw = result.stderr.strip() or result.stdout.strip() or "sem detalhes adicionais"
        detail = raw[:200]
        return {
            "status": "error",
